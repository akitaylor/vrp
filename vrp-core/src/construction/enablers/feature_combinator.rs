...
fn get_solution_state(solution_ctx: &SolutionContext) -> (usize, usize, usize, Vec<(usize, usize, usize)>) {
    (
...
fn get_stale_routes(solution_ctx: &SolutionContext) -> Vec<(usize, usize, usize)> {
    solution_ctx
        .routes
        .iter()
        .enumerate()
